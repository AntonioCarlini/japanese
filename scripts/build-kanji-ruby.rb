...
kanji_with_unique_keyword.each() {
  |k|
  puts("    :\"#{kanji_keyword(k)}\" => [#{k.heisig()}, #{k.unicode()}, [\"#{k.onyomi().join('","')}\"], [\"#{k.kunyomi().join('","')}\"], [\"#{k.nanori().join('","')}\"], [\"#{k.english().join('","')}\"], #{k.jouyou()}, #{k.jlpt()}],")
}
puts("    }")
puts("    @kanji_map = {}  # Stores initialized Kanji objects")
puts("  end")
puts("")
puts("  # When a kanji is requested, init the map entry the first time. Always return the required Kanji object")
puts("  def kanji(key)")
puts("    begin")
puts("      @kanji_map[key] ||= Kanji.new(*@kanji_data[key.to_sym()])")
puts("    rescue")
puts("      $stderr.puts(\"FastKanji: UNKNOWN kanji for [#\{key}]\")")
puts("      return nil")
puts("    end")
puts("  end")
puts("end")
