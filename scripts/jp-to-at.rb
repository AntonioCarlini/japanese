...
def process_single_line(line, kjuc)
  ...
  ji.each() {
    ...
    if current_mode == :kanji && style != :kanji
      ...
      when :hiragana
        print("@HI{{#{pending}}}") unless pending.empty?()
      when :furigana
        ## TEMP TODO raise unless style == :hiragana || style == :furigana_end # will accumulate below but must be hiragana or end of furigana
...
if ARGV[0] == "--file" && ARGV[1]
  ...
  File.foreach(filename) do |line|
    process_single_line(line.chomp(), kjuc)
  end
else
  line = ARGV.join()
  process_single_line(line, kjuc)
end
