...
def processing()
  ...
  # REF is special too and must be processed before anything else.
  # Recursive REFs make no sense so do not cater for them.
  begin
    file_text.gsub!(/@REF\{\{([^\}]+)\}\}/m) {
      |what|
      ref = $1
  ...
  while !to_handle.nil?() && !to_handle.empty?()
    debug("Starting with to_handle.size = #{to_handle.size()}")    
    m = /(#{OPENING_REGEXP}|#{CLOSING_REGEXP})/ixm.match(to_handle)
    if !m.nil?()
      text = m.pre_match()
      current = m[0]
      to_handle = m.post_match().dup()
      debug("Found something to process")    
      
      if stack.empty?()
        print(text)
      else
        debug("Stacking text [#{text}]")    
        stack << Text.new(text)
      end

      if current =~ /#{CLOSING_REGEXP}/
        debug("Processing last command [#{current}]")
        ...
        if stack.empty?()
          ...
          debug("Stacking processed text")    
          # TODO - if the first item is Text then prepend to to_handle and process again
          stack << collapse_result(result)
        end

      else
        debug("Stacking command [#{current}]")
        if current =~ /@(.*)#{OPENING_BRACKETS}/
          ...
          if command =~ /^(N|V|S)(\d*)$/
            ...
            if op.nil?()
              debug("No OP found for [#{command}] in [#{current}]") # should be a raise
            else
      ...
      debug("Finished handling text")
      print(to_handle)
      break
    end
    debug("Loop end: to_handle.size=#{to_handle.size()}")
  end

  unless stack.empty?()
    debug("Finished with non-empty stack")
    dump_stack(stack)
